...
function updateHTML(data, userInfo) {
    ...
    // Bilibili Video Link Regex
    const BILIBILI_REG = /<a\s+href="https:\/\/www\.bilibili\.com\/video\/((av[\d]{1,10})|(BV[\w]{10}))\/?">.*<\/a>/g;
    
    // Netease Music Link Regex
    const NETEASE_MUSIC_REG = /<a\s+href="https:\/\/music\.163\.com\/.*id=([0-9]+)".*>.*<\/a>/g;
    
    // QQ Music Link Regex
    const QQMUSIC_REG = /<a\s+href="https:\/\/y\.qq\.com\/.*\/([0-9a-zA-Z]+)(\.html)?".*>.*<\/a>/g;
    
    // QQ Video Link Regex
    const QQVIDEO_REG = /<a\s+href="https:\/\/v\.qq\.com\/.*\/([a-zA-Z0-9]+)\.html".*>.*<\/a>/g;
    
    // Spotify Link Regex
    const SPOTIFY_REG = /<a\s+href="https:\/\/open\.spotify\.com\/(track|album)\/([\s\S]+)".*>.*<\/a>/g;
    
    // Youku Video Link Regex
    const YOUKU_REG = /<a\s+href="https:\/\/v\.youku\.com\/.*\/id_([a-zA-Z0-9=]+)\.html".*>.*<\/a>/g;
    
    // YouTube Video Link Regex
    const YOUTUBE_REG = /<a\s+href="https:\/\/www\.youtube\.com\/watch\?v=([a-zA-Z0-9]{11})".*>.*<\/a>/g;
    ...
    for (const item of data) {
        ...
        const formattedMemos = item.name.split('/')[1];
        ...
        memoResult += `<li class="timeline">
            <div class="memos__content" style="--avatar-url: url(${userInfo.avatarurl})">
                <div class="memos__text">
                    <div class="memos__userinfo">
                        <a href=${userInfo.userurl} target="_blank" >
                            <div>${userInfo.memoname}</div>
                        </a>
                        <div>
                        ...
                        <div class="memos__id">@${userInfo.memousername}</div>
                    </div>
                    <p>${memoContREG}</p>
                </div>
                <div class="memos__meta">
                    <small class="memos__date">${relativeTime} • From「<a href="${memosHost}/m/${formattedMemos}" target="_blank">Memos</a>」${locationHtml}</small>
                </div>
